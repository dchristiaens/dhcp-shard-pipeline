...
class Image (object):
    ...
    def save(self, filename):
        ...
        # write image header
        with io.open(filename, 'w', encoding='latin-1') as f:
            f.write(u'mrtrix image\n')
            f.write(u'dim: ' + self._to_csv(self.shape) + '\n');
            f.write(u'vox: ' + self._to_csv(self.vox) + '\n')
            f.write(u'layout: ' + self._to_csv(self.layout) + '\n')
            f.write(u'datatype: ' + _dtdict_inv[self.dtype.descr[0][1]] + '\n')
            f.write(u'transform: ' + self._to_csv(self.transform[0,:]) + '\n')
            f.write(u'transform: ' + self._to_csv(self.transform[1,:]) + '\n')
            f.write(u'transform: ' + self._to_csv(self.transform[2,:]) + '\n')
            if self.labels is not None:
                f.write(u'labels: ' + self._to_csv(self.labels) + '\n')
            if self.units is not None:
                f.write(u'units: ' + self._to_csv(self.units) + '\n')
            for comment in self.comments:
                f.write(u'comments: ' + comment + '\n')
            for k in [] if self.grad is None else range(self.grad.shape[0]):
                f.write(u'dw_scheme: ' + self._to_csv(self.grad[k,:]) + '\n')
            f.flush()
            offset = f.tell() + 13
